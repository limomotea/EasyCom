...
# nameEntered = ttk.Entry(win, width=10,textvariable=name)  # 创建一个文本框，定义长度为12个字符长度，并且将文本框中的内容绑定到上一句定义的name变量上，方便clickMe调用
#
...
numberChosen.grid(column=10, row=1)  # 设置其在界面中出现的位置  column代表列   row 代表行
